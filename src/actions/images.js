...
export const getImages = (activeBookmarkId) => async dispatch => {
    try {
        const res = await api.get(`/image/${activeBookmarkId}`);
...
export const uploadImages = (images, group) => async dispatch => {
    try {
        ...
        // Get presigned urls
        const resUrls = await api.get('/upload/image?count=' + images.length);
        ...
        const res = await api.post('/image', resUrls.data.keys )
...
export const deleteImages = (ids) => async dispatch => {
    try {
        const res = await api.delete('/image', { data: ids });
